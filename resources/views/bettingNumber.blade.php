                ...
                <div class="row mt-3">
                    <div class="col-md-12 boder-danger me-5 pe-5">
                        <div class="row mb" style="margin-bottom: 30px;">
                            <form action="{{ route('filter-batting-number') }}" method="post">
                                @csrf
                                <div class="row mb-3">
                                    <div class="col-sm-1">
                                    ...
                                    <div class="col-md-1 " style="margin-left:-12px;  margin-top:40px;">
                                        <a class="btn bg-light shadow-lg" href="{{ route('betting-number') }}">Reset</a>
                                    </div>
                            </form>
                            <!--  -->
                            <div class="col-sm-2" style="position:relative;top:40px;">
                                ...
                                <div class="Click-here" data-target="#mymodel" data-toggle="modal">
                                    <button class="btn bg-light shadow-lg" type="button"
                                        ...
                                        data-bs-target="#exampleModal">+ Add New Number</button>
                                </div>
                                <!-- Modal -->
                                <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content" id="store-modal-content">
                                            ...
                                            <form action="{{ route('betting-number-store') }}" id="storeform"
                                                method="post">
                                                @csrf
                                                <div class="modal-body">
                                                    ...
                                                    <select name="betting_location_id" id="location"
                                                        class="form-control">
                                                        <option selected disabled>Select Location</option>
                                                        @foreach ($locations as $data)
                                                            <option value="{{ $data->id }}">
                                                                {{ $data->betting_location }}</option>
                                                        @endforeach
                                                    </select>
                                                    <label for="number" class="form-lable">Betting No.</label>
                                                    <input type="number" name="betting_number" id="number"
                                                        class="form-control">
                                                </div>
        ...
        $(document).ready(function() {
            $('.delete-location').click(function(event) {
                ...
                if (confirm('Are you sure you want to delete this Number?')) {
                    $.ajax({
                        url: '/delete-betting-number/' + locationId,
                        type: 'DELETE',
                        headers: {
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.format').change(function() {
                ...
                $.ajax({
                    url: '{{ url('/update-betting-number-status') }}',
                    type: 'POST',
                    data: {
    ...
    <script>
        $(document).ready(function() {
            $('.edit-location').click(function() {
                var locationId = $(this).data('location-id');
                $.ajax({
                    url: '/betting-number/' + locationId + '/edit',
                    type: 'GET',
                    success: function(response) {
                        ...
                        $('#edit_betting_location').val(response.betting_location_id);
                        $('#edit_total_number').val(response.betting_number);
                        $('#editLocationForm').attr('action', '/betting-number/' + locationId);
